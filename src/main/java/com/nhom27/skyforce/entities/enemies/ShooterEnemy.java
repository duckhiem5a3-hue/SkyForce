...
public class ShooterEnemy extends EnemyObject {
    ...
    private static List<EnemyBullet> enemyBulletsList = new ArrayList<>(); // đặt là static để việc cập nhật đạn không
                                                                           // phụ thuộc vào việc enemy đã chết chưa
    ...
    public void update() {
        ...
        if (x <= 0) {
            setPos(Main.WIDTH - sizeX, startHeight);
        } else if (x >= Main.WIDTH - sizeX) {
            setPos(0, startHeight);
        }
